...
class Admin_JySystemTicket extends MY_Controller {
	...
	public function index() {
        ...
        if (!empty($input)) {
            ...
            $system = $this->JySystem_service->get_list(array("system_type"=>5));
        }

        $channel = json_decode(JySystem_model::CHANNEL_ARRAY,true);
        $this->assign("channel",$channel);
        $this->assign("system", $system['rows']);
        $this->view("admin/jysystemticket/index");
	}
	...
	public function edit() {
		...
		if (!empty($input)) {
            ...
            if($success['success']){
                alert($success['msg'],"/Admin_JySystemTicket/index?system_class=".$system_class);
            }else{
                alert($success['msg']);
            }
		}
        ...
        $this->assign("channel",$channel);
		$this->view("admin/jysystemticket/edit");
	}
	...
	public function add() {
		...
		if (!empty($input)) {
			...
			$success = $this->JySystem_service->addoredit($input);
            if($success['success']){
                alert($success['msg'],"/Admin_JySystemTicket/index");
            }else{
                alert($success['msg']);
            }
        ...
        $this->assign("type",$type);
        $this->assign("channel",$channel);
		$this->view("admin/jysystemticket/add");
	}
